...
const char *WINDOWTITLE = { "Final Project -- Tanya Khemani" };
...
float Time;
int Airballnum;
int Airballmove[50][30][2];
int Airballfall[50][30];
float Airballpos[50][30][3];
int Airballgen = 0;
...
Animate( )
{
	...
	if (ms%20 == 0)
	{
		...
		Airballnum = random_gen(15, 30);
		for (int i = 0; i <= Airballnum; i++)
		{
			Airballmove[Airballgen][i][0] = random_gen(-2, 2);
			Airballmove[Airballgen][i][1] = random_gen(-2, 2);
			Airballpos[Airballgen][i][0] = random_gen(-50, 50);
			Airballpos[Airballgen][i][1] = random_gen(30, 40);
			Airballpos[Airballgen][i][2] = random_gen(-50, 50);
			Airballfall[Airballgen][i] = random_gen(-2.5, -0.5);
		}
		Airballgen++;
		if (Airballgen >= 50)
			Airballgen = 0;
...
Display( )
{
	...
	//lights
	SetSpotLight(GL_LIGHT2, 10., -sqrt(42.5*42.5 - 27.*27.) - 7.5, 10., Light1Dirx, Light1Diry, Light1Dirz, 1., 0., 0.);
	SetSpotLight(GL_LIGHT3, 10., -sqrt(42.5*42.5 - 27.*27.) - 7.5, -10., Light2Dirx, Light2Diry, Light2Dirz, 0., 1., 0.);
	SetSpotLight(GL_LIGHT4, -10., -sqrt(42.5*42.5 - 27.*27.) - 7.5, -10., Light3Dirx, Light3Diry, Light3Dirz, 1., 0., 1.);
	SetSpotLight(GL_LIGHT5, -10., -sqrt(42.5*42.5 - 27.*27.) - 7.5, 10., Light4Dirx, Light4Diry, Light4Dirz, 0., 1., 0.);
	...
	else if (LightSwitch == 1)
	{
		...
		glEnable(GL_LIGHT4);
		glEnable(GL_LIGHT5);
		if (LightParaSwitch == 1)
		{
			glEnable(GL_LIGHT6);
		}
		else if (LightParaSwitch == 0)
		{
			glDisable(GL_LIGHT6);
		}
	}
	...
	if (AirballSwitch == 1)
	{
			...
			for (int j = 0; j < Airballnum; j++)
			{

				glColor3f(random_colors[j*3], random_colors[j*3+1], random_colors[j*3+2]);
				for (int q = 0; q < Airballgen; q++)
				{
					Airballpos[q][j][0] += Time * Airballmove[q][j][0];
					Airballpos[q][j][1] += Time * Airballfall[q][j];
					Airballpos[q][j][2] += Time * Airballmove[q][j][1];
					if (sqrt(Airballpos[q][j][0] * Airballpos[q][j][0] + Airballpos[q][j][1] * Airballpos[q][j][1] + Airballpos[q][j][2] * Airballpos[q][j][2]) <= 38)
					{
						glPushMatrix();
						glTranslatef(Airballpos[q][j][0], Airballpos[q][j][1], Airballpos[q][j][2]);
...
InitMenus( )
{
	...
	int Airballmenu = glutCreateMenu( DoAirballMenu );
	glutAddMenuEntry( "Off",  0 );
	glutAddMenuEntry( "On",  1 );
	...
	glutAddSubMenu(   "ColoredBalls",  Airballmenu);
...
InitGraphics( )
{
	...
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1, &UranusRingTex);
	glBindTexture(GL_TEXTURE_2D, UranusRingTex);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	Light1Dirx = random_gen(-300, 300);
	Light1Diry = random_gen(-300, 300);
	Light1Dirz = random_gen(-330, 300);
	Light2Dirx = random_gen(-330, 300);
	Light2Diry = random_gen(0, 300);
	Light2Dirz = random_gen(-30, 330);
	Light3Dirx = random_gen(-330, 300);
	Light3Diry = random_gen(0, 330);
	Light3Dirz = random_gen(-303, 300);
	Light4Dirx = random_gen(-300, 300);
	Light4Diry = random_gen(0, 330);
	Light4Dirz = random_gen(-300, 300);
