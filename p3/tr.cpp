	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort)
			{
				...
				/*if (lat < 0)
				{
					p->s = (lng + M_PI) / (2.*M_PI) *cos((lat + M_PI / 2) / TimeCo);
				}
				else
				{
					p->s = (lng + M_PI) / (2.*M_PI) *cos((M_PI / 2 - lat) / TimeCo);
				}*/
...
Display( )
{
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Texture Mapping" );
...
InitGraphics( )
{
	...
	//Texture = BmpToTexture("simpletexture.bmp", &Width, &Height);
