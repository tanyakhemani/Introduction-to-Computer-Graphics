...
Display()
{
	...
	glutSolidSphere(2., 10, 10);					//moving sphere
	glPopMatrix();
	...
	glColor3f(1, 1, 1);
	glTranslatef(5, 5, 0.);
	MjbSphere(0.2, 10, 10);					   		//small sphere with white color
	glPopMatrix();

	glPushMatrix();
	if (Light2On) {
		SetSpotLight(GL_LIGHT1, 5, 5, 0, -1, 0, 0,  1, 1, 1);    //white spot light
	...
	SetMaterial(1, 0, 0, 1.);
	glColor3f(0., 0., 1.);
	MjbSphere(0.2, 10, 10);							//small sphere with blue color
	glPopMatrix();

	glPushMatrix();
	if (Light1On) {
		SetPointLight(GL_LIGHT2, -3, 3, 0, 0., 0., 1.); 			//blue  light    
	}
	else {
		glDisable(GL_LIGHT2);
	}
	...
	glColor3f(1., 0., 0.);							//small sphere with red color
	MjbSphere(0.2, 10, 10);
	glPopMatrix();

	glPushMatrix();
	if (Light0On) {
	SetPointLight(GL_LIGHT3, 3., -3, 10 * sin(2 * M_PI*Time), 1., 0., 0.); //moving red light    
	}
